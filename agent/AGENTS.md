...
Step 7: 记录 & 推送 → memory/YYYY-MM-DD-picks.md + 消息通知
```
...
## 能力二：个股分析（被动响应）—— 子Agent调度架构

**触发**："分析XX" / "看看XXXXXX" / "深度调研XX" / "拆解XX"

> **架构原则**：主Session只做调度（接收指令→分派任务→整合结果），研究任务由隔离子Agent执行。

### 子Agent调度流程（所有维度的统一入口）

**主Session执行步骤**：

```
1. 意图识别
   ├── 识别研究类型：深度分析(维度一) / 快速分析(维度二) / 超级深度调研(维度三)
   ├── 识别股票代码/名称
   └── 识别用户特殊要求（如有）
...
2. 研究任务构建
   ...
   │   ├── 数据源配置
   │   └── 交付要求（输出格式、报告标题等）
   └── 生成任务label：deep-research-{股票代码}

3. 分派任务（使用 sessions_spawn）
   ├── context: "isolated"（干净研究环境）
   ├── model: deepseek/deepseek-v4-pro
   ├── thinking: xhigh
   └── timeout: 根据深度级别设置（见下表）

4. 等待结果（使用 sessions_yield）
   └── 主Session在此暂停，不占上下文
...
2. 执行研究（按workflow）
   ├── 维度一 → deep-dive-workflow.md（13维度）
   ├── 维度二 → analysis-method.md（六维）
   └── 维度三 → super-deep-research-workflow.md（5-Task）
   └── Task 0: 准备与环境检查（/compact在此执行）

3. 生成报告
   ├── 构建JSON数据
   ├── 调用 report-generator.py 生成HTML
   └── 部署到Cloudflare Pages（如失败则发送HTML文件）

4. 返回结果（使用 sessions_send）
   └── 向父Session发送结构化结果：
       {
         "status": "completed",
         "report_url": "Cloudflare链接或HTML文件路径",
         "summary": "核心结论摘要",
         "key_findings": ["关键发现1", "关键发现2"],
         "stock_code": "股票代码",
         "report_type": "深度分析/快速分析/超级深度调研"
       }
```
...
| 研究维度 | 子Agent超时 | 说明 |
|----------|------------|------|
| 快速分析（维度二） | 10分钟 | 六维框架，轻量快速 |
| 深度分析（维度一） | 60分钟 | 13维度，Level 2-3 |
| 超级深度调研（维度三） | 120分钟 | 5-Task全流程，Level 4 |

### 维度一：深度分析（默认维度）

> **执行方式**：通过子Agent隔离执行，主Session仅调度

**主Session流程**：
1. 构建研究任务prompt，加载 `references/deep-dive-workflow.md`
2. 调用 `sessions_spawn` 分派到隔离子Agent
3. `sessions_yield` 等待结果
4. 整合输出回复用户
...
6. 通过 `sessions_send` 返回结果给父Session
...
**主Session流程**：
1. 构建研究任务prompt，加载 `references/analysis-method.md`
2. 调用 `sessions_spawn` 分派到隔离子Agent
3. `sessions_yield` 等待结果（超时10分钟）
4. 整合输出回复用户
...
7. 通过 `sessions_send` 返回结果给父Session
...
**主Session流程**：
1. 构建研究任务prompt，加载 `references/super-deep-research-workflow.md`
2. 调用 `sessions_spawn` 分派到隔离子Agent
3. `sessions_yield` 等待结果（超时120分钟）
4. 整合输出回复用户
5. 更新 `memory/stock-docs.json` 注册表
...
3. 六段式框架 + 13维度实战框架全覆盖
4. 输出：HTML报告（标题格式：`🔬🔬 {股票名}({代码}) 超级深度调研报告`）
5. 通过 `sessions_send` 返回结果给父Session
...
Step 4: 用户通知 → 仅🔴紧急级实时推送（消息通知），🟡每日汇总一次
...
| 个股超级深度调研 | `super-deep_{6位代码}_{YYYYMMDD}.html` | 5-Task超级深度调研 |
...
**JSON数据格式**（输入给report-generator时需包含report_type字段）：

```json
{
    "report_type": "deep-research",
    "stock_code": "600519",
    "title": "贵州茅台深度分析",
    "content_markdown": "...",
    "dashboard": [...]

...
**【强制前置】**：每次开始新个股深度调研前，**必须先执行 `/compact` 压缩会话上下文**

**前置条件**：用户提供目标股票名称或代码

**执行步骤**：
0. **【强制】先执行 `/compact` 命令压缩会话上下文**（释放之前调研的上下文记忆）
1. 确认股票代码/名称
...
**核心目标**：生成HTML报告（动态图表+专业样式）

**输出架构**：
- **HTML报告**：使用 report-generator 生成，支持动态图表和ECharts可视化
...
**Cloudflare部署失败时**：直接发送HTML文件给用户，不要放弃。
